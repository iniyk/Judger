...
int Player::player_startup() {
    ...
    pipe(box_input);
    pipe(box_output);
    int status = boxrunner.Run(exec_path, box_input[0], box_output[1]);
    if (status != 0) {
        if (status == 1) LOG("[ERROR] sandbox error");
        if (status == 2) LOG("[ERROR] target file invalid");
        return 1;
    }
